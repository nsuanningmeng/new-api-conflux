...
func GetTopUpInfo(c *gin.Context) {
	...
	enableConfluxAPI := isConfluxAPITopUpEnabled()
	if enableConfluxAPI {
		hasConfluxAPI := false
		for _, method := range payMethods {
			if method["type"] == model.PaymentMethodConfluxAPI {
				hasConfluxAPI = true
				break
			}
		}
		...
		if !hasConfluxAPI {
			payMethods = append(payMethods, map[string]string{
				"name":      "Conflux",
				"type":      model.PaymentMethodConfluxAPI,
				"color":     "rgba(var(--semi-green-5), 1)",
				"min_topup": strconv.Itoa(setting.ConfluxAPIMinTopUp),
			})
		}
	}
	...
	data := gin.H{
		...
		"enable_confluxapi_topup":          enableConfluxAPI,
		...
		"confluxapi_min_topup":    setting.ConfluxAPIMinTopUp,
