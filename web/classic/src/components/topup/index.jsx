...
const TopUp = () => {
  ...
  const [enableConfluxAPITopUp, setEnableConfluxAPITopUp] = useState(false);
  ...
  const requestAmountByPayment = async (payment, value) => {
    ...
    if (payment === 'confluxapi') {
      return getConfluxAPIAmount(value);
    }
  ...
  const preTopUp = async (payment) => {
    ...
    } else if (payment === 'confluxapi') {
      if (!enableConfluxAPITopUp) {
        showError(t('管理员未开启 Conflux 充值！'));
        return;
      }
    } else {
  ...
  const onlineTopUp = async () => {
    ...
    } else if (payWay === 'confluxapi') {
      if (amount === 0) {
        await getConfluxAPIAmount();
      }
    } else {
      // 普通支付处理
      if (amount === 0) {
    ...
    try {
      ...
      if (payWay === 'stripe') {
        ...
        res = await API.post('/api/user/confluxapi/pay', {
          amount: parseInt(topUpCount),
        });
      } else {
        // 普通支付请求
      ...
      if (res !== undefined) {
        ...
        if (message === 'success') {
          if (payWay === 'stripe') {
            ...
            const paymentUrl =
              data?.checkout_url || data?.payment_url || data?.qr_code;
            if (paymentUrl) {
  ...
  const getConfluxAPIAmount = async (value) => {
    if (value === undefined) {
      value = topUpCount;
    }
    setAmountLoading(true);
    try {
      const res = await API.post('/api/user/confluxapi/amount', {
        amount: parseInt(value),
      });
      if (res !== undefined) {
        const { message, data } = res.data;
    ...
    try {
      ...
      if (success) {
        ...
        try {
          ...
          const enableConfluxAPITopUp =
            data.enable_confluxapi_topup || false;
            ...
            : enableStripeTopUp
              ...
              : enableWaffoTopUp
                ...
                : enableWaffoPancakeTopUp
                  ...
                  : enableConfluxAPITopUp
                    ? data.confluxapi_min_topup
                  : 1;
          ...
          setEnableConfluxAPITopUp(enableConfluxAPITopUp);
    ...
    <div className='w-full max-w-7xl mx-auto relative min-h-screen lg:min-h-0 mt-[60px] px-2'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <RechargeCard
          ...
          enableConfluxAPITopUp={enableConfluxAPITopUp}
