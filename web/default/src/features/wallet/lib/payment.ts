...
/**
 * Check if payment method is ConfluxAPI
 */
export function isConfluxAPIPayment(paymentType: string): boolean {
  return paymentType === PAYMENT_TYPES.CONFLUXAPI
}
...
export function getDefaultPaymentType(topupInfo: TopupInfo | null): string {
  ...
  if (topupInfo.enable_confluxapi_topup) {
    return PAYMENT_TYPES.CONFLUXAPI
  }
...
export function getMinTopupAmount(topupInfo: TopupInfo | null): number {
  ...
  if (topupInfo.enable_confluxapi_topup) {
    return topupInfo.confluxapi_min_topup || DEFAULT_MIN_TOPUP
  }
