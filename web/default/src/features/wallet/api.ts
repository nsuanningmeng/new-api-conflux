...
import type {
  ...
  ConfluxAPIPaymentRequest,
  ConfluxAPIPaymentResponse,
} from './types'
...
/**
 * Calculate payment amount for ConfluxAPI payment
 */
export async function calculateConfluxAPIAmount(
  request: AmountRequest
): Promise<AmountResponse> {
  const res = await api.post('/api/user/confluxapi/amount', request, {
    skipBusinessError: true,
  } as Record<string, unknown>)
  return res.data
}
...
/**
 * Request ConfluxAPI payment
 */
export async function requestConfluxAPIPayment(
  request: ConfluxAPIPaymentRequest
): Promise<ConfluxAPIPaymentResponse> {
  const res = await api.post('/api/user/confluxapi/pay', request, {
    skipBusinessError: true,
  } as Record<string, unknown>)
  return res.data
}
